 ...
 * 1.00       12 May 2020     Doug Humberd     Sets the Print Total Hours value on Invoices
 * 1.05       27 May 2020     Doug Humberd     Removed condition "if item == OPENAIR_INTEGRATION"
 * 1.06       01 Jun 2020     Doug Humberd     Updated internal id from 'custcol_print_total_hours' to 'custcol_print_total_hours_bytask_byem'
 * 1.10       15 Jul 2020     Doug Humberd     Updated to run in Edit mode
 * 1.15       11 Apr 2022     Doug Humberd     Changed to run as beforeSubmit (not afterSubmit)
 *
 */
 ...
 * @returns {Void}
 */
function is_pth_beforeLoad(type, form, request){
    try {
        //is_pth_beforeLoadScript(type, form, request);
    } catch (e) {
        is_pth_logError(e);
        throw e;
    }
 ...
 * @returns {Void}
 */
function is_pth_beforeSubmit(type) {
    nlapiLogExecution('DEBUG', 'Before Submit - Type: ' + type, '');
    try {
    	is_pth_setPrintTotalHours(type);
        //is_pth_beforeSubmitScript(type);
    } catch (e) {
        is_pth_logError(e);
        throw e;
    }
 ...
 * @returns {Void}
 */
function is_pth_afterSubmit(type) {
    nlapiLogExecution('DEBUG', 'After Submit - Type: ' + type, '');
    try {
        //is_pth_setPrintTotalHours(type);
      	//is_pth_afterSubmitScript(type);
    } catch (e) {
        is_pth_logError(e);
        throw e;
    }
 ...
 * @returns {Void}
 */
function is_pth_setPrintTotalHours(type){
	...
	nlapiLogExecution('DEBUG', 'setPrintTotalHours UE', 'START');
    ...
    //var rec = nlapiLoadRecord(nlapiGetRecordType(), invId);//REMOVED VERSION 1.15

    //var lineCount = rec.getLineItemCount('item');//CHANGED VERSION 1.15
	...
	*/
    ...
    for (var i = 1; i <= lineCount; i++){
    	...
    	//Clear out Existing Value, if any, in the 'Total Hours By Task By Employee' field
    	//rec.setLineItemValue('item', 'custcol_print_total_hours_bytask_byem', i, '');//CHANGED VERSION 1.15
    	nlapiSetLineItemValue('item', 'custcol_print_total_hours_bytask_byem', i, '');
    	
    	//var item = rec.getLineItemValue('item', 'item', i);//CHANGED VERSION 1.15
    	var item = nlapiGetLineItemValue('item', 'item', i);
    	//var printTimeEntry = rec.getLineItemValue('item', 'custcol_print_time_entry', i);//CHANGED VERSION 1.15
    	var printTimeEntry = nlapiGetLineItemValue('item', 'custcol_print_time_entry', i);
    	...
    	if (printTimeEntry == 'T'){
    		...
    		//assgnUser = rec.getLineItemValue('item', 'custcol_oa_wbs_assignees', i);//CHANGED VERSION 1.15
    		assgnUser = nlapiGetLineItemValue('item', 'custcol_oa_wbs_assignees', i);
    		//taskName = rec.getLineItemValue('item', 'custcol_oa_wbs_task_name', i);//CHANGED VERSION 1.15
    		taskName = nlapiGetLineItemValue('item', 'custcol_oa_wbs_task_name', i);
    		...
    		//Create Arrays to tally / store Print Total Hours Information, and which line to write it to
    		if (arrAssgnTask.indexOf(combined) < 0) {
  	          	//nlapiLogExecution('debug', i, '1');
  	          	var ind = arrAssgnTask.length || 0;
  	          	nlapiLogExecution('DEBUG', 'ind', ind);
  	          	arrAssgnTask[ind] = combined;
  	          	//arrPrtTotHrs[ind] = parseFloat(rec.getLineItemValue('item', 'custcol_quantity_for_customer', i));//CHANGED VERSION 1.15
  	          	arrPrtTotHrs[ind] = parseFloat(nlapiGetLineItemValue('item', 'custcol_quantity_for_customer', i));
  	          	arrLine[ind] = i;
  	          	continue;
  	        } else {
  	        	//nlapiLogExecution('debug', i, '2');
  	        	var index = arrAssgnTask.indexOf(combined);
  	        	nlapiLogExecution('DEBUG', 'index', index);
  	        	//arrPrtTotHrs[index] += parseFloat(rec.getLineItemValue('item', 'custcol_quantity_for_customer', i));//CHANGED VERSION 1.15
  	        	arrPrtTotHrs[index] += parseFloat(nlapiGetLineItemValue('item', 'custcol_quantity_for_customer', i));
  	        	continue;
  	        }
    ...
    for (var j = 0; j < arrAssgnTask.length; j++) {

    	nlapiLogExecution('DEBUG', 'Set Print Total Hours on Line ' + arrLine[j], arrPrtTotHrs[j]);
    	//rec.setLineItemValue('item', 'custcol_print_total_hours_bytask_byem', arrLine[j], arrPrtTotHrs[j]);//CHANGED VERSION 1.15
    	nlapiSetLineItemValue('item', 'custcol_print_total_hours_bytask_byem', arrLine[j], arrPrtTotHrs[j]);
    ...
    //nlapiSubmitRecord(rec, false, true);//REMOVED VERSION 1.15
    
    nlapiLogExecution('DEBUG', 'setPrintTotalHours UE', 'FINISH');
	
		
	}catch (err) {
	    nlapiLogExecution('error', 'setPrintTotalHours', JSON.stringify(handleException(err)));
	}
