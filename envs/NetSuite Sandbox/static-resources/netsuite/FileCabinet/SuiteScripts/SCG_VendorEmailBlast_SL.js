 ...
 * 1.00				04 Apr 2022		Doug Humberd		Executes a Scheduled Script to Send Emails to Vendors from Specific Subsidiaries with varying wording / attachments
...
const EMAIL_ATTACH_FOLDER_ARCHIVE = '4073210';//Folder: Vendor Email Blast Attachments - Archive
 ...
 */
function is_veb_vendorEmailBlast(request, response){
	nlapiLogExecution('DEBUG', 'Vendor Email Blast SL', 'START');
    if (request.getMethod()=='GET') {
        try	{
        	
        	var form = nlapiCreateForm('Send Vendor Email Blast');
            ...
            //fld = form.addField('custpage_subsidiary', 'multiselect' , 'Exclude Subsidiaries', 'subsidiary');
        	fld = form.addField('custpage_subsidiary', 'multiselect' , 'Include Subsidiaries', 'subsidiary');
            ...
            fld = form.addField('custpage_author', 'select', 'Email Author', 'employee');
            fld.setDefaultValue(emailauthor);
            fld.setMandatory(true);
            ...
            fld = form.addField('custpage_reprocess', 'checkbox', 'Process Only Previously Failed Vendors');
        ...
        try {
        	...
        	emailauthor = request.getParameter('custpage_author');
        	nlapiLogExecution('DEBUG', 'emailauthor', emailauthor);
        	...
        	var scriptSched = nlapiScheduleScript('customscript_scg_vendoremailblast_ss', 'customdeploy_scg_vendoremailblast_ss', {'custscript_veb_sub_ids': splitSubsids, 'custscript_veb_template_id': template, 'custscript_veb_reprocess': reprocess, 'custscript_veb_emailauthor': emailauthor});
            nlapiLogExecution('DEBUG', 'Deployment Scheduled: ' + ' Status: ' + scriptSched, '');
            //nlapiSetRedirectURL('TASKLINK', 'LIST_SCRIPTSTATUS', null, null, {'scripttype': is_veb_getScriptInternalId('customscript_scg_sendemailtocusts_ss')});
            nlapiSetRedirectURL('TASKLINK', 'LIST_SCRIPTSTATUS', null, null, {'scripttype': is_veb_getScriptInternalId('customscript_scg_vendoremailblast_ss')});
 ...
 * @returns {Integer}
 */
function is_veb_getScriptInternalId(scriptId) {
