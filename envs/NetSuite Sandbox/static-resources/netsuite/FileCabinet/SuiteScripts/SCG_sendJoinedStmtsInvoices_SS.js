 ...
 * 1.05             03 Mar 2020     Doug Humberd        Updated to remove "Jet" restrictions, Add New Field for Criteria - 'Disable Statement Email Notification'
 ...
 * Global Variables
 */
//var is_jsi_context = nlapiGetContext();
 ...
 * @returns {Void}
 */
function is_jsi_sendJoinedStmtsInvs(type) {
	
	nlapiLogExecution('DEBUG', 'is_jsi_sendJoinedStmtsInvs', 'START');
	
	// Initialize variables
	var minRecId = 0;
	
	var customerIds = is_jsi_getCustomerIds(minRecId);
	var count = 0;
	var custId;
	var filesToDelete;
	...
	while (customerIds && customerIds.length > 0){
		...
		is_jsi_scheduledBatch(customerIds, function (customerId) {
			try{
				// Initialize variables
				filesToDelete = [];
				...
				// Get Parameters
				var emailTemplate = nlapiGetContext().getSetting('SCRIPT', 'custscript_jsi_email_template');
				var emailAuthor = nlapiGetContext().getSetting('SCRIPT', 'custscript_jsi_email_author');
				...
				if (blnce > 0){
					nlapiLogExecution('DEBUG', 'custId: ' + custId, 'Create Statement PDF - Prim Subsidiary');
					...
					if (invSearch != null && invSearch.length <= 75){
						...
						//for (var i = 0; i < 1; i++){
							...
							nlapiLogExecution('DEBUG', 'invId: ' + invId, 'Create Invoice PDF - Prim Subsidiary');
				...
				if(recipient != null){
					...
					for (var n = 1; submachineCount != 0 && n <= submachineCount; n++){
						...
						if (balance > 0 && isPrimeSub == 'F'){
							...
							nlapiLogExecution('DEBUG', 'custId: ' + custId, 'Create Statement PDF - Non Prim Subsidiary');
							...
							if (invSearch != null && invSearch.length <= 75){
								...
								//for (var x = 0; x < 1; x++){
									...
									//Create the Invoice PDF
									nlapiLogExecution('DEBUG', 'invId: ' + invId, 'Create Invoice PDF - Non Prim Subsidiary');
									var invPdfFile = nlapiPrintRecord('TRANSACTION', invId, 'PDF', null);
		...
		customerIds = is_jsi_getCustomerIds(minRecId);
 ...
 * @returns {Void}
 */
function is_jsi_scheduledBatch(arr, proc) {
 ...
 * @returns {nlobjSearch}
 */
function is_jsi_getCustomerIds(minRecId) {
	// Define filters
