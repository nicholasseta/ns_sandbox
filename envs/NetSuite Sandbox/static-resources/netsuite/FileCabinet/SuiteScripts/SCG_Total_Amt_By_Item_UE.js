 ...
 * 1.00       01 Jun 2020     Doug Humberd     Sets the Total Amount by Item value on Invoices
 * 1.05       20 Dec 2021     Doug Humberd     Added logic to also set Total TAX by Item to existing Total Amount by Item function
 ...
 * @returns {Void}
 */
function is_tai_beforeLoad(type, form, request){
    try {
        //is_tai_beforeLoadScript(type, form, request);
    } catch (e) {
        is_tai_logError(e);
        throw e;
    }
 ...
 * @returns {Void}
 */
function is_tai_beforeSubmit(type) {
    nlapiLogExecution('DEBUG', 'Before Submit - Type: ' + type, '');
    try {
    	is_tai_setTotalAmtByItem(type);
        //is_tai_beforeSubmitScript(type);
    } catch (e) {
        is_tai_logError(e);
        throw e;
    }
 ...
 * @returns {Void}
 */
function is_tai_afterSubmit(type) {
    nlapiLogExecution('DEBUG', 'After Submit - Type: ' + type, '');
    try {
        //is_tai_setTotalAmtByItem(type);
      	//is_tai_afterSubmitScript(type);
    } catch (e) {
        is_tai_logError(e);
        throw e;
    }
 ...
 * @returns {Void}
 */
function is_tai_setTotalAmtByItem(type){
	...
	nlapiLogExecution('DEBUG', 'setTotalAmtByItem UE', 'START');
	...
	*/
    ...
    for (var i = 1; i <= lineCount; i++){
    	...
    	//rec.setLineItemValue('item', 'custcol_total_amount_by_item', i, '');//CHANGED VERSION 1.10
    	nlapiSetLineItemValue('item', 'custcol_total_amount_by_item', i, '');
    	
    	//Clear out Existing Value, if any, in the 'Total Tax By Item' field
    	//rec.setLineItemValue('item', 'custcol_total_tax_by_item', i, '');//CHANGED VERSION 1.10
    ...
    for (var j = 0; j < arrItems.length; j++) {
    	...
    	//rec.setLineItemValue('item', 'custcol_total_amount_by_item', arrLine[j], arrTotAmt[j]);//CHANGED VERSION 1.10
    	...
    	//rec.setLineItemValue('item', 'custcol_total_tax_by_item', arrLine[j], arrTotTax[j]);//CHANGED VERSION 1.10
    ...
    nlapiLogExecution('DEBUG', 'setTotalAmtByItem UE', 'FINISH');
	    ...
	    nlapiLogExecution('error', 'setTotalAmtByItem', JSON.stringify(handleException(err)));
	}
