 ...
 * 1.00       23 Feb 2021     Doug Humberd     Sets the Total Amount by Tax Code value on Invoices
 * 1.01       20 Dec 2021     Doug Humberd     Updated to always run in Edit mode
 *
 */
 ...
 * @returns {Void}
 */
function is_tabtc_beforeLoad(type, form, request){
    try {
        //is_tabtc_beforeLoadScript(type, form, request);
    } catch (e) {
        is_tabtc_logError(e);
        throw e;
    }
 ...
 * @returns {Void}
 */
function is_tabtc_beforeSubmit(type) {
    nlapiLogExecution('DEBUG', 'Before Submit - Type: ' + type, '');
    try {
        //is_tabtc_beforeSubmitScript(type);
    } catch (e) {
        is_tabtc_logError(e);
        throw e;
    }
 ...
 * @returns {Void}
 */
function is_tabtc_afterSubmit(type) {
    nlapiLogExecution('DEBUG', 'After Submit - Type: ' + type, '');
    try {
        is_tabtc_setTotalAmtByTaxCode(type);
      	//is_tabtc_afterSubmitScript(type);
    } catch (e) {
        is_tabtc_logError(e);
        throw e;
    }
 ...
 * @returns {Void}
 */
function is_tabtc_setTotalAmtByTaxCode(type){
	...
	nlapiLogExecution('DEBUG', 'setTotalAmtByTaxCode UE', 'START');
	...
	*/
    ...
    for (var i = 1; i <= lineCount; i++){
    	...
    	//Clear out Existing Value, if any, in the 'Total Amount By Tax Code' field
    	rec.setLineItemValue('item', 'custcol_scg_total_amount_by_taxcode', i, '');
		...
		if (arrTaxcodes.indexOf(taxCode) < 0) {
          	...
          	arrTotAmt[ind] = parseFloat(rec.getLineItemValue('item', 'amount', i));
          	arrLine[ind] = i;
          	continue;
        } else {
        	...
        	arrTotAmt[index] += parseFloat(rec.getLineItemValue('item', 'amount', i));
        	continue;
        }
    ...
    for (var j = 0; j < arrTaxcodes.length; j++) {
    	...
    	nlapiLogExecution('DEBUG', 'Set Total Amount by Tax Code on Line ' + arrLine[j], arrTotAmt[j]);
    	rec.setLineItemValue('item', 'custcol_scg_total_amount_by_taxcode', arrLine[j], arrTotAmt[j]);
    ...
    nlapiLogExecution('DEBUG', 'setTotalAmtByTaxCode UE', 'FINISH');
	
		
	}catch (err) {
	    nlapiLogExecution('error', 'setTotalAmtByTaxCode', JSON.stringify(handleException(err)));
	}
