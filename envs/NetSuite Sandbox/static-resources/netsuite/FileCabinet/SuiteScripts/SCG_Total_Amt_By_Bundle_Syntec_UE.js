 ...
 * 1.00       03 Mar 2021     Doug Humberd     Sets the Syntec Total by Bundle value on Invoices (only for lines where Item = Syntec)
 ...
 * @returns {Void}
 */
function is_stb_beforeLoad(type, form, request){
    try {
        //is_stb_beforeLoadScript(type, form, request);
    } catch (e) {
        is_stb_logError(e);
        throw e;
    }
 ...
 * @returns {Void}
 */
function is_stb_beforeSubmit(type) {
    nlapiLogExecution('DEBUG', 'Before Submit - Type: ' + type, '');
    try {
    	is_stb_setSyntecTotalByBundle(type);
        //is_stb_beforeSubmitScript(type);
    } catch (e) {
        is_stb_logError(e);
        throw e;
    }
 ...
 * @returns {Void}
 */
function is_stb_afterSubmit(type) {
    nlapiLogExecution('DEBUG', 'After Submit - Type: ' + type, '');
    try {
        //is_stb_setSyntecTotalByBundle(type);
      	//is_stb_afterSubmitScript(type);
    } catch (e) {
        is_stb_logError(e);
        throw e;
    }
 ...
 * @returns {Void}
 */
function is_stb_setSyntecTotalByBundle(type){
	...
	nlapiLogExecution('DEBUG', 'setSyntecTotalByBundle UE', 'START');
	...
	*/
    ...
    for (var i = 1; i <= lineCount; i++){
    	...
    	//Clear out Existing Value, if any, in the 'Syntec Total By Bundle' field
    	//rec.setLineItemValue('item', 'custcol_syntec_total_by_bundle', i, '');//CHANGED VERSION 1.05
    	nlapiSetLineItemValue('item', 'custcol_syntec_total_by_bundle', i, '');
    	...
    	if (item != SYNTEC){
    		nlapiLogExecution('DEBUG', 'Item is NOT Syntec', 'SKIP LINE');
    		continue;
    	}
    ...
    for (var j = 0; j < arrSynBundles.length; j++) {
    	...
    	nlapiLogExecution('DEBUG', 'Set Syntec Total by Bundle on Line ' + arrLine[j], arrTotAmt[j]);
    	//rec.setLineItemValue('item', 'custcol_syntec_total_by_bundle', arrLine[j], arrTotAmt[j]);//CHANGED VERSION 1.05
    	nlapiSetLineItemValue('item', 'custcol_syntec_total_by_bundle', arrLine[j], arrTotAmt[j]);
    ...
    nlapiLogExecution('DEBUG', 'setSyntecTotalByBundle UE', 'FINISH');
	
		
	}catch (err) {
	    nlapiLogExecution('error', 'setSyntecTotalByBundle', JSON.stringify(handleException(err)));
	}
